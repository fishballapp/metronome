...
fun SlowTempoAlert(onProceed: () -> Unit, onDismiss: () -> Unit, tempo: Long) {
  AppAlert(
    AppAlertDetail(
      title = "Slow Tempo",
      paragraphs = listOf(
        "The tempo ($tempo BPM) is... unusually slow.",
        "Are you sure to proceed?",
      ),
      icon = Icons.Rounded.Warning,
      actions = listOf(
        { ProceedActionChip(onProceed) },
        { DismissActionChip("Back", onDismiss) }
      )
    )
  )
}
...
fun FastTempoAlert(onProceed: () -> Unit, onDismiss: () -> Unit, tempo: Long) {
  AppAlert(
    AppAlertDetail(
      title = "Fast Tempo",
      paragraphs = listOf(
        "The tempo ($tempo BPM) is... quite fast.",
        "Are you sure to proceed?",
      ),
      icon = Icons.Rounded.Warning,
      actions = listOf(
...
fun PlayStoreActionChip() {
  ...
  Chip(
    ...
    secondaryLabel = {
      Text("on Google Play Store", maxLines = 1)
    },
    onClick = {
      val webUri =
...
private fun AppAlert(
  ...
  Alert(
    ...
    contentPadding =
    PaddingValues(start = 10.dp, end = 10.dp, top = 8.dp, bottom = 80.dp),
  ) {
    items(details.paragraphs) {
      Text(
        it,
        textAlign = TextAlign.Center,
      )
